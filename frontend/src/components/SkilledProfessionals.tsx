...
const SkilledProfessionals = () => {
  ...
  return (
    <section
      ref={sectionRef}
      className={`py-20 px-4 relative transition-all duration-700 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}
    >
      ...
      <div className="max-w-7xl mx-auto">
        <div className="mb-12 text-center">
          <h2 className="text-4xl lg:text-5xl font-black text-amber-100 mb-2">{t('skilled.heading')}</h2>
          <p className="text-amber-100/70 text-lg">{t('skilled.subheading')}</p>
        </div>
        ...
        <div className="relative group overflow-hidden">
          ...
          <div className="overflow-hidden">
            <div className="flex gap-6 transition-transform duration-700 ease-in-out" style={{ transform: `translateX(-${currentIndex * 25.4}%)` }}>
                ...
                >
                  <div className="relative bg-[#2d1a0a]/40 backdrop-blur-md border border-amber-500/20 rounded-2xl overflow-hidden">
                    ...
                    <div className="absolute bottom-0 left-0 right-0 p-6">
                      <h3 className="text-2xl font-bold text-transparent bg-gradient-to-br from-amber-200 to-amber-400 bg-clip-text mb-2">{professional.name}</h3>
                      <div className="inline-flex items-center text-amber-400/60 hover:text-amber-400/80 transition-colors duration-100 hover:translate-x-1.5 transition-transform">
                        <span className="text-sm font-semibold mr-1">{t('skilled.view_cta')}</span>
